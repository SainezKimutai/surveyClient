...
export class ReportsComponent implements OnInit {
  ...
  //  tslint:disable
    ...
    downloadPdf(){
      ...
      var HTML_Width = this.reportTemplate.nativeElement.offsetWidth;
      var HTML_Height =  this.reportTemplate.nativeElement.offsetHeight;
    ...
    headerTemplate.then((headerCanvas) => {
      ...
      // pdf.addImage(headerCanvas, 'JPG', top_left_margin, top_left_margin,canvas_image_width,canvas_image_height);
      pdf.addImage(headerCanvas, 'PNG', 15, 60, canvas_image_width, 80);

      pdf.text(510, 520, `${this.myCompany.companyName}`);
      pdf.setFontSize(15);
      pdf.text(440, 550, `Report generated by Tactive Admin`);
      pdf.setFontType("italic");
      pdf.setFontSize(15);
      pdf.text(490, 580, 'CopyRight @2020');
      pdf.setFontSize(15);
      pdf.addPage();
      
      summaryGraph.then((summaryGraphCanvard) => {
        pdf.addImage(summaryGraphCanvard, 'PNG', 10, 250, canvas_image_width, 400);
        // pdf.addImage(summaryGraphCanvard, 'JPG', top_left_margin, top_left_margin,canvas_image_width,canvas_image_height);
        pdf.addPage();
